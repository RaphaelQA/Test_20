...
def director_dao():
    ...
    raf = Director(id=2, name='raf')
    den = Director(id=3, name='den')

    director_dao.get_one = MagicMock(return_value=jonh)
    director_dao.get_all = MagicMock(return_value=[jonh, raf, den])
    director_dao.create = MagicMock(return_value=Director(id=3, name='Qup'))
